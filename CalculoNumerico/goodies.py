...
sdirname = "./.tmp/"
dest_dirname = "../on_server/CalculoNumerico/" + sys.argv[1] + "/"
...
for index, f in enumerate (lfiles):
    ...
    head1 += "Livro, Cálculo Numérico, Métodos Numéricos, Análise Numérica"
    codeIn = []
    if (sys.argv[1] == 'livro-sci'):
        codeIn = 'Scilab'
    elif (sys.argv[1] == 'livro-oct'):
    ...
    if (s!=-1):
        ...
        text = text.replace("+++paginaNoGitHub+++",
                            "https://github.com/reamat/CalculoNumerico/blob/master/"+auxText)
        s = text.find("#srcPath:")
        auxText = text[s+9:]
        e = auxText.find("#")
    ...
    text = text.replace("../livro-sci/#pagina","../livro-sci/"+f+".html")
