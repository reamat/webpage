...
#title
title = "<title>Transformada de Laplace - Um Livro Colaborativo"
s = text.index('<title>')
e = text.index('</title>')
text = text.replace(text[s:e], title)

#titleHead
text = text.replace('<h2 class="titleHead">Transformada de Laplace<br />',
                    '<h2 class="titleHead">Transformada de Laplace<br /><small>')
...
for index, f in enumerate (lfiles):
    ...
    head1 += "Livro, Transformada de Laplace, Transformadas Integrais"
    if (len(kw) != 0):
        head1 += ", " + kw
    head1 += "'>\n"
    ...
    #book title
    text = text.replace("+++tituloDoLivro+++","Transformada de Laplace - Um Livro Colaborativo")    
    ...
    if (s!=-1):
        ...
        text = text.replace("+++paginaNoGitHub+++",
                            "https://github.com/reamat/TransformadasIntegrais/blob/master/TransformadaLaplace/"+auxText)
        s = text.find("#srcPath:")
        auxText = text[s+9:]
        e = auxText.find("#")
    ...
    #set version change
    text = text.replace("+++urlsumario+++","../livro-tl/main.html")
    text = text.replace("+++urlpdf+++","../livro-tl/livro.pdf")
