...
sdirname = "./.tmp-cfuv/"
dest_dirname = "../on_server/Calculo/livro-cfuv/"
...
#title
title = "<title>Cálculo de funções de uma variável - Um Livro Colaborativo"
s = text.index('<title>')
e = text.index('</title>')
text = text.replace(text[s:e], title)

#titleHead
text = text.replace('<h2 class="titleHead">Cálculo de funções de uma variável<br />',
                    '<h2 class="titleHead">Cálculo de funções de uma variável<br /><small>')
...
for index, f in enumerate (lfiles):
    ...
    head1 += "Livro, Cálculo diferencial e integral, funções de uma variável, "
    if (len(kw) != 0):
        head1 += ", " + kw
    head1 += "'>\n"
    ...
    #book title
    text = text.replace("+++tituloDoLivro+++","Cálculo de funções de uma variável - Um Livro Colaborativo")
    ...
    if (s!=-1):
        ...
        text = text.replace("+++paginaNoGitHub+++",
                            "https://github.com/reamat/Calculo/blob/master/CFUV/"+auxText)
        s = text.find("#srcPath:")
        auxText = text[s+9:]
        e = auxText.find("#")
    ...
    #set version change
    text = text.replace("+++urlsumario+++","../livro-cfuv/main.html")
    text = text.replace("+++urlpdf+++","../livro-cfuv/livro.pdf")
